...
class OvercookedGymEnv(Env):
    ...
    def set_env_layout(self, env_index=None, layout_name=None, base_env=None, horizon=None):
        ...
        if base_env is None:
            ...
            self.mp = MotionPlanner.from_pickle_or_compute(self.mdp, COUNTERS_PARAMS, force_compute=False)
        ...
        self.num_players = 1 if 'single' in self.layout_name else 2
        self.terrain = self.mdp.terrain_mtx
        self.prev_subtask = [Subtasks.SUBTASKS_TO_IDS['unknown'], Subtasks.SUBTASKS_TO_IDS['unknown']]
        self.env.reset()
        self.valid_counters = [self.env.mdp.find_free_counters_valid_for_player(self.env.state, self.mp, i) for i in
                               range(self.num_players)]  # range(2
        self.reset()
    ...
    def reset(self, p_idx=None):
        if p_idx is not None:
            ...
            self.p_idx = np.random.randint(self.num_players)
