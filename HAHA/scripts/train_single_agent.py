...
def train_selfplay_agent(args, training_steps=1e7, n_checkpoints=0, save_filename=None):
    ...
    selfplay_trainer = RLAgentTrainer([], args, selfplay=True, name=name, seed=678, use_frame_stack=False,
                                      use_lstm=False, use_cnn=False, use_conf=True, fcp_ck_rate=checkpoint_rate)
...
def plot_training_data(data_file):
    ...
    training_data["avg_conf_normalized"] = training_data['conf_mean'] / training_data['seen_states']

    for column in training_data.columns.values:
        if column in ['Unnamed: 0', 'timestep', 'time']:
            continue

        plt.plot(training_data['timestep'], training_data[column])
