...
from overcooked_ai_py.agents.agent import AgentPair, RandomAgent, GreedyHumanModel, AgentGroup
...
class AgentEvaluator(object):
    ...
    def evaluate_oai_agent(self, agent, num_games, game_length=None, start_state_fn=None, metadata_fn=None, metadata_info_fn=None, display=False, dir=None,
        ...
        agent_group = AgentGroup(agent)

        if native_eval:
            agent_group.agents[0].set_idx(0, self.env)
            agent_group.agents[0].set_encoding_params(self.env.mdp, self.env.horizon)

            return self.env.get_rollouts(agent_group, num_games=num_games, display=display, dir=dir,
            ...
            agent_group.agents[0].set_idx(0, horizon_env)
            agent_group.agents[0].set_encoding_params(horizon_env.mdp, horizon_env.horizon)

            return horizon_env.get_rollouts(agent_group, num_games=num_games, display=display, dir=dir,
                                            display_phi=display_phi,
                                            info=info, metadata_fn=metadata_fn, metadata_info_fn=metadata_info_fn)
