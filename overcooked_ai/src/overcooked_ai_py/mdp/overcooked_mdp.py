...
class OvercookedGridworld(object):
            ...
            if curr_subtask in ['get_onion_from_dispenser', 'get_plate_from_dish_rack',
                ...
                if 'from_counter' in curr_subtask:
                    ...
                    if curr_subtask == 'get_onion_from_counter':
                        ...
                        obj_name = 'soup'
                    possible_counters = self.find_free_counters_valid_for_player(start_state, mlam.motion_planner, p_idx)
    ...
    def find_free_counters_valid_for_player(self, state, mp, p_idx):
        """Finds all empty counter locations that are accessible to both players"""
        player = state.players[p_idx]
        free_counters = self.get_empty_counter_locations(state)
        free_counters_valid_for_both = []
        for free_counter in free_counters:
            goals = mp.motion_goals_for_pos[free_counter]
            if any([mp.is_valid_motion_start_goal_pair(player.pos_and_or, goal) for goal in goals]):
                free_counters_valid_for_both.append(free_counter)
        return free_counters_valid_for_both
    ...
    def lossless_state_encoding(self, overcooked_state, goal_objects=None, horizon=400, debug=False):
        ...
        assert self.num_players in [1, 2], "Functionality has to be added to support encondings for > 2 players"
